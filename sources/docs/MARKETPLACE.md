...
- The login and registration password inputs each have an accessible eye control. It swaps the supplied closed/open-eye image and changes the field between `password` and `text` without changing its value.
...
- Pages: `homepage`, `login`, `register`, `about`, `visits_list`, `create_visits`, `my_items`, `create_items`, `user_profile`. `navbar.js` renders the shared navigation on every page: **Le mie visite / I miei item / About us / Navigator / Account / Log in-out**. On an authenticated mobile viewport, the Account and Logout links are replaced by an avatar button whose accessible dropdown contains both actions; desktop keeps the two direct links. There is no museum-authoring UI here — museums are seeded via `scripts/load-museum.js` (see [AGENTS.md](../AGENTS.md) §5), not created by end users.
- **Responsive layer:** every active Marketplace page loads `stylesheets/responsive.css` *after* its page stylesheet. It is intentionally limited to cross-page safeguards: no horizontal overflow, a touch-scrollable wrapped navbar, a one-column-safe visit grid at 320px, and a correct top offset for the Item editor below the mobile navbar. At `1440px+`, only working areas and card grids are constrained (up to 1640px) so laptop/TV screens remain readable while editorial backgrounds stay full-bleed. Keep page-specific rules in their existing stylesheet and put shared viewport fixes there.
- **Screen-reader semantics:** the shared navigation exposes its landmark and current page; asynchronous grids/status lines use `aria-busy` and live status roles; editor modals name their title/description, receive focus when opened and restore it on close. The chapter and Item-detail tabs use linked `tab`/`tabpanel` IDs with arrow-key navigation. Custom controls must remain native buttons where possible; the Item editor's duration choices use a pressed-button group, so its editable textareas preserve their native textbox role and an accessible name.
...
- The visit editor writes both the flat `sequence` required by item execution and ordered `blocks` required by question sections. Chapters (`.visit-block`) render as tabs — `#chapter-tabs` lists one pill per chapter, while its `+` add control is a sibling in `.chapter-tabs-row` so it does not violate the tablist semantics; only the chapter carrying `.is-active-block` is shown below. `create_visit.js`'s `setActiveBlock()`/`renderChapterTabs()` own that state. ⚠️ Because only one chapter is in view at a time, drag/reorder is scoped to the open chapter's own list — there is no cross-chapter drag anymore (previously possible when every section sat side by side in a scrollable row). Route directions between consecutive Content stops still work within a chapter. Question sections support open and multiple-choice prompts with a correct answer for the latter.
- The visit picker lists **museum Content**, one card per `Content.universalId`, instead of duplicating a Content once for every available Item. This includes artworks, artists, movements and places returned by `GET /museums/:id/contents`. Content already present anywhere in the visit is removed from the picker; while replacing a stop, its current Content remains eligible but Content used by every other stop stays excluded. Adding a Content creates its visit row without silently choosing an Item; the row's required `.item-choice-select`, beside the Item edit buttons, then lists the owned, purchased and purchasable/public Items for that Content. The projected wallet cost is recalculated when the selection changes, and choices that exceed the available balance are disabled. The select is also disabled while the catalogue loads or when the Content has no Items; in the latter case the pencil button opens `create_items.html` to create the first one. Restored `sessionStorage` state is kept until catalogue hydration succeeds, so a transient fetch error does not destroy the selected IDs. **Cambia contenuto** swaps the Content row in place and preserves the stop's directions.
- The expanded Content card (`.item-accordion-body`, built in `creaEdAggiungiItem()`) is three siblings in a row: a bigger `.artwork-card` image (260px, was 190px), a read-only **Scheda Content** info panel (`.item-info-panel` — Content author/year/type plus selected Item author/license/target/price), then the compact `.item-route-panel` directions textarea (`min-height` cut from 70px to 44px). The compact row toggles this body on click through `aria-expanded`; hover and focus no longer open it. The pencil remains the explicit route to the Item editor. `updateItemSpecificRow()` refreshes Item-specific values after a dropdown change; `ensureRoutePanel()` still appends the route panel last, so DOM order stays image → info → directions.
- `#select-item-modal` has a single scrollbar: `.visit-builder-modal` is a flex column with `overflow-y: hidden` when this modal is open, and only `.items-scroll-container` scrolls (`flex: 1 1 auto; min-height: 0`). Before, both the modal and the inner Content grid had independent `overflow-y: auto`, producing two nested scrollbars whenever the list was tall. This override is scoped to `#select-item-modal .visit-builder-modal` — the other two modals sharing that class (`section-type-modal`, `cancel-confirm-modal`) are unaffected.
- `create_items.html`'s left metadata column is intentionally short: **Licenza, Autore item, Prezzo, Visibilità, Target** only. Visibility is an explicit **Pubblico / Privato** radio choice. Target is a required free-text string (1–120 characters), not a fixed audience select; legacy seeded values such as `tourist` are still rendered with their localized labels. `ANNO`, `ID UNIVERSALE`, `LINGUA` and the `VENDITE`/`RICAVI` read-outs were dropped — `year`/`universalId` were display-only (never sent in the save payload; `Item` has no `year` field at all), and text language is now hardcoded to `'it'` in the save payload instead of a per-item selector (Italian-only copy, per [AGENTS.md](../AGENTS.md) §3).
- The three tone sections (`children`/`standard`/`expert` DOM ids mapping to `easy`/`medium`/`complex`) are headed **Tono facile / Tono medio / Tono complesso** and **Easy / Medium / Complex** in English, matching the Navigator. Each has a one-line hint naming the expected lexicon. `create_item.js`'s `applyTailoredPlaceholders()` fills all 9 textareas with a placeholder built from the open artwork's title (`urlTitle`) plus per-cell guidance (`TONE_WRITING_GUIDE` × `DURATION_WRITING_GUIDE`: lexicon/voice per tone, word-count target per duration) instead of one generic sentence reused everywhere.
...
- **I miei item** starts from the Content grid. Opening a Content defaults to three explicit tabs: **Marketplace** lists public Items by other authors that the user has not bought yet (price, details and purchase action); **My Items** contains authored Items with edit/delete controls; **Purchased Items** contains prior purchases. A **Create new Item** action stays visible in the detail header in every tab. The shared tab panel keeps the stable `#items-detail-list` id: changing that id during rendering previously broke every tab switch and every Content opened after the first. A successful Marketplace purchase moves the Item out of the first set and opens Purchased Items immediately. Content cards deliberately have no hover movement animation.
