...
- first use the requested `3s` / `15s` / `45s` entry in the current UI language;
...
- The effect is keyed on the enabled flag, text and resolved text language. Next /
  ...
  utterance and start the new one. Switching `3s → 15s` mid-sentence therefore cuts the
  3s read short — **desired**.
